...
const openProfile = async (name) => {
  ...
  {/* SEARCH BAR */}
  <div className={`max-w-6xl mx-auto mb-16 rounded-full px-6 py-4 shadow-md ${darkMode ? "bg-slate-800" : "bg-gray-50"}`}>
    <input
      type="text"
      placeholder="What would you like to learn today?"
      value={search}
      onChange={(e) => setSearch(e.target.value)}
      className={`w-full bg-transparent outline-none text-lg ${darkMode ? "text-white placeholder-slate-400" : "text-gray-900 placeholder-gray-400"}`}
    />
  </div>

  {/* MENTOR CARDS */}
  {filteredMentors.length === 0 ? (
    <p className={`text-center ${darkMode ? "text-slate-400" : "text-gray-500"}`}>No mentors found.</p>
  ) : (
      ...
      {filteredMentors.map((mentor) => (
        <div
          ...
          className={`relative rounded-3xl p-6 transition-all duration-300 border ${
            darkMode
              ? "bg-slate-800 border-slate-700 shadow-md hover:shadow-xl hover:-translate-y-1"
        ...
        >
          ...
          <p className={`text-sm mb-3 ${darkMode ? "text-slate-300" : "text-gray-500"}`}>Teaches:</p>
