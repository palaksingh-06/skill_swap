      ...
      <div className="max-w-7xl mx-auto px-6 py-14 grid md:grid-cols-3 gap-8">
        ...
        {matches.map((u) => {
          ...
          return (
            <div
              key={u._id}
              onClick={() => navigate(`/profile/${u._id}`)} // ✅ OPEN PUBLIC PROFILE
              className={`rounded-2xl p-6 shadow-md transition hover:-translate-y-1 cursor-pointer ${
                darkMode ? "bg-slate-800" : "bg-white"
              }`}
            >
              {/* NAME */}
              <div className="mb-4">
                <h3 className="font-semibold text-lg hover:text-teal-500">
                  {u.name}
                </h3>
                <p className="text-sm text-gray-500">
                  {u.location || "Remote"}
